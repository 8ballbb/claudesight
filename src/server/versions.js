...
export function storeRoot(home = os.homedir()) {
  return path.join(home, '.claudesight', 'versions')
}
...
function ensureDir(dir, home) {
  ...
  const chain = [path.join(home, '.claudesight'), storeRoot(home), dir]
...
export function deleteVersion(target, id, home = os.homedir(), forceMechanism = null) {
  ...
  const box = path.join(home, '.claudesight', '.trashing',
    `claudesight-version-${path.basename(target)}-${id}`)
